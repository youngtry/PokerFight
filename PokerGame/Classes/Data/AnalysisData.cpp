...
bool AnalysisData::init(std::string filename){
    ...
    while (rootEle) {
        ...
        while (childEle) {
            ...
            }else if (strcmp(childEle->Name(), "StoryCount") == 0){
                int count = atoi(childEle->GetText());
                log("storycount = %d",count);
                
            }else if (strcmp(childEle->Name(), "StoryKey") == 0){
                const XMLElement* keyEle = childEle->FirstChildElement();
                
                while (keyEle) {
                    if(strcmp(keyEle->Name(),"KeyName") == 0){
                        const XMLAttribute* keyname = keyEle->FirstAttribute();
